...
```bash
claude plugin marketplace update stm-ai-sci
claude plugin uninstall ai-scientist@stm-ai-sci --scope project
claude plugin install ai-scientist@stm-ai-sci --scope project
```
...
- Enable Codex rescue: `codex.rescue_on_stuck: true` in config
...
### Codex Review Fails
- Check Codex is installed: `which codex`
- Check authentication: `codex login status`
- If Codex is unavailable, the Claude review (steps 1-8) still runs independently
